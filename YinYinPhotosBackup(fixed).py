...
def getPhotos(albumName, albumId):  #获取图片的函数
    ...
    #通过提取相册中第一张图片的photoId从而进入包含该相册所有相片的最终页页面finalPage
    ...
    finalPage=ungzip(urllib.request.urlopen(finalLink).read()).decode()
    photoLinkRe = re.compile(r'"largeurl":"(.*?)",')
    photoLinkList = re.findall(photoLinkRe, finalPage)[1:] #去除重复的首张相片
...
#从albumsList 和 albumsId中剔除相片数为0的项 从而避免下载异常
for i in photoCount:
    if i=='0':
        index = photoCount.index(i)
        albumsList.pop(index)
        albumsId.pop(index)
